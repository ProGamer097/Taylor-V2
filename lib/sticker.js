...
async function sticker8(img, url) {
    img = img ? img : await (await fetch(url)).arrayBuffer();
    const output = await imageToWebp(img);
    if (!output) throw "Error Convert";
    return output;
}

async function sticker9(img, url) {
    img = img ? img : await (await fetch(url)).arrayBuffer();
    const output = await videoToWebp(img);
    if (!output) throw "Error Convert";
    return output;
}

async function sticker10(path, packname = global.packname, author = global.author) {
    let buffer;
    try {
