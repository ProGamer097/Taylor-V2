   ...
   sendButtonMessages: {
    async value(jid, text = '', footer = '', buffer, buttons, copy, urls, quoted, options) {
        ...
        const msgs = generateWAMessageFromContent(jid, messageContent, {
                    ...
                    quoted: quoted,
                    contextInfo: {
                        mentionedJid: await conn.parseMention(text + footer)
                    },
                    ephemeralExpiration: ephemeral,
                    ...options
                });
        ...
        sendButtonMessage: {
            async value(jid, buttons, body, footer, header, subtitle, quoted, options = {}) {
                ...
                const msgs = generateWAMessageFromContent(jid, messageContent, {
                    ...
                    quoted: quoted,
                    contextInfo: {
                        mentionedJid: await conn.parseMention(body + footer)
                    },
                    ephemeralExpiration: ephemeral,
                    ...options
                });
    ...
    async value(jid, title, text, buttonText, listSections, quoted, options = {}) {
        ...
        const msgs = generateWAMessageFromContent(jid, messageContent, {
                    ...
                    quoted: quoted,
                    contextInfo: {
                        mentionedJid: await conn.parseMention(text + title)
                    },
                    ephemeralExpiration: ephemeral,
                    ...options
                });
        ...
        quoted: {
            get() {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {
                    ...
                    device: {
  get() {
    return getDevice(this.vM.key?.id || this.id) || os.platform() === 'android' ? 'Android Device' : ['win32', 'darwin', 'linux'].includes(os.platform()) ? 'Desktop' : 'Unknown Device';
  },
  enumerable: true,
},
isBot: {
  get() {
    const idBot = this.vM.key?.id || this.id;
  ...
  enumerable: true,
        ...
        device: {
  get() {
    return getDevice(this.key?.id || this.id) || os.platform() === 'android' ? 'Android Device' : ['win32', 'darwin', 'linux'].includes(os.platform()) ? 'Desktop' : 'Unknown Device';
  },
  enumerable: true,
},
isBot: {
  get() {
    const idBot = this.key?.id || this.id;
    return ["BAE5", "WA"].some(k => idBot.includes(k) && this.sender !== this.conn?.user.jid);
  },
  enumerable: true,
}
