...
⏲️ *Duration Timestamp:* ${duration.timestamp ?? 'Tidak diketahui'}
⌛ *Duration Seconds:* ${duration.seconds ?? 'Tidak diketahui'}
⌚ *Ago:* ${v.ago}
👀 *Views:* ${formatNumber(v.views)}
👤 *Author Name:* ${author.name ?? 'Tidak diketahui'}
🔗 *Author URL:* ${author.url ?? 'Tidak diketahui'}
   `.trim();
            case "canal":
                return `
🔖 *${v.name}* (${v.url})
